...
print("STARTING_CONVERTER_V2", flush=True)
import pandas as pd
import json
...
def convert_to_json():
    ...
    try:
        ...
        print("DataFrame created from records.", flush=True)
        ...
        if 'quantity' in df.columns:
            ...
            print("Warning: quantity column missing in DF", flush=True)
            
        # 2. Normalize Categories
        print("Normalizing categories...", flush=True)
        if 'category' in df.columns:
            # Drop if exists (impossible here but safety)
            if 'category_norm' in df.columns:
                df.drop(columns=['category_norm'], inplace=True)
                
            df['category_norm'] = df['category'].apply(normalize_category_name)
        else:
             print("Warning: category column missing in DF", flush=True)
        ...
        # Check col existence before groupby
        if 'product' in df and 'category_norm' in df:
            ...
            print(f"Missing grouping columns. Columns: {df.columns}", flush=True)
